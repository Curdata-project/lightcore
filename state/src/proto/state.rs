...
impl<'a> MessageWrite for State<'a> {
    fn get_size(&self) -> usize {
        ...
        } + if self.state == Cow::Borrowed(b"") {
            0
        } else {
            1 + sizeof_len((&self.state).len())
        } + if self.owner == Cow::Borrowed(b"") {
            0
        } else {
            1 + sizeof_len((&self.owner).len())
        } + if self.lock == Cow::Borrowed(b"") {
            0
        } else {
            1 + sizeof_len((&self.lock).len())
        } + if self.valid == Cow::Borrowed(b"") {
            0
        } else {
            1 + sizeof_len((&self.valid).len())
        } + self
            .msg
            .as_ref()
    ...
    fn write_message<W: WriterBackend>(&self, w: &mut Writer<W>) -> Result<()> {
        if self.size != 0u64 {
            w.write_with_tag(8, |w| w.write_uint64(*&self.size))?;
        }
        if self.state != Cow::Borrowed(b"") {
            w.write_with_tag(18, |w| w.write_bytes(&**&self.state))?;
        }
        if self.owner != Cow::Borrowed(b"") {
            w.write_with_tag(26, |w| w.write_bytes(&**&self.owner))?;
        }
        if self.lock != Cow::Borrowed(b"") {
            w.write_with_tag(34, |w| w.write_bytes(&**&self.lock))?;
        }
        if self.valid != Cow::Borrowed(b"") {
            w.write_with_tag(42, |w| w.write_bytes(&**&self.valid))?;
        }
...
impl<'a> MessageWrite for SignedState<'a> {
    fn get_size(&self) -> usize {
        0 + if self.id == Cow::Borrowed(b"") {
            ...
            .as_ref()
            .map_or(0, |m| 1 + sizeof_len((m).get_size()))
            + if self.witness == Cow::Borrowed(b"") {
                0
            } else {
                1 + sizeof_len((&self.witness).len())
            }
            + if self.signature == Cow::Borrowed(b"") {
                0
            } else {
    ...
    fn write_message<W: WriterBackend>(&self, w: &mut Writer<W>) -> Result<()> {
        ...
        if self.witness != Cow::Borrowed(b"") {
            w.write_with_tag(26, |w| w.write_bytes(&**&self.witness))?;
        }
        if self.signature != Cow::Borrowed(b"") {
            w.write_with_tag(34, |w| w.write_bytes(&**&self.signature))?;
        }
        if let Some(ref s) = self.msg {
